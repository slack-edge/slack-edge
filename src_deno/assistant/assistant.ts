...
import { AssistantThreadContextStore } from "./thread-context-store.ts";
 ...
 */
export type AssistantThreadStartedHandler<E extends SlackAppEnv> =
  AssistantEventLazyHandler<AssistantThreadStartedEvent, E>;
 ...
 * @template E - The Slack app environment type
 */
export type AssistantThreadContextChangedHandler<E extends SlackAppEnv> =
...
export class Assistant<E extends SlackAppEnv> {
  ...
  threadContextChangedHandler: EventLazyHandler<
    ...
    this.threadStartedHandler = async (req) => {
      try {
        if (isAssitantThreadEvent(req.body)) {
          const request = req as unknown as AssistantThreadEventRequest<
            E,
            AssistantThreadStartedEvent
          >;
        ...
        console.error(
          `Failed to execute threadStartedHandler listener: ${
            (e as Error).stack
          }`,
        );
      }
        ...
        if (isAssitantThreadEvent(req.body)) {
          ...
          if (options.threadContextChanged) {
            ...
            await context.saveThreadContextStore({
              ...request.payload.assistant_thread.context,
            });
          }
        }
      } catch (e: unknown) {
        console.error(
          `Failed to execute threadContextChangedHandler listener: ${
            (e as Error).stack
          }`,
        );
      }
  ...
  threadStarted(handler: AssistantThreadStartedHandler<E>) {
    this.threadStartedHandler = async (req) => {
      try {
        if (isAssitantThreadEvent(req.body)) {
          await handler(
            req as unknown as AssistantThreadEventRequest<
              E,
              AssistantThreadStartedEvent
            >,
          );
