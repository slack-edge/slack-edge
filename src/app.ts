...
export class SlackApp<E extends SlackEdgeAppEnv | SlackSocketModeAppEnv> {
  ...
  private generateListenerSuggestion(body: SlackRequestBody): string {
    if (body.type === PayloadType.EventsAPI && body.event) {
      ...
      return (
        `Suggestion: Register an event listener for '${eventType}' events:\n\n` +
      ...
      return (
        `Suggestion: Register a slash command listener for '${command}':\n\n` +
      ...
      return (
        `Suggestion: Register a global shortcut listener for '${callbackId}':\n\n` +
      ...
      const callbackId = body.callback_id || "your_callback_id";
      return (
        `Suggestion: Register a message shortcut listener for '${callbackId}':\n\n` +
      ...
      return (
        `Suggestion: Register a block action listener for '${actionId}':\n\n` +
        suggestion +
        `  return "";\n` +
        `}, async (req) => {\n` +
        `  // Handle block action\n` +
      ...
      const callbackId = body.view?.callback_id || "your_callback_id";
      return (
        `Suggestion: Register a view closed listener for '${callbackId}':\n\n` +
      ...
      return (
        `Suggestion: Register an app rate limited listener:\n\n` +
    ...
    return (
      `Suggestion: Check your request payload and register the appropriate listener.\n` +
    ...
    if (isRequestSignatureVerified) {
      ...
      try {
        ...
        if ("name" in error && error.name === "AuthorizeError") {
          const responseOrError: Response | AuthorizeError = await this.authorizeErrorHandler({
